...
import warnings
import glob  # Added for handling split files
...
def load_exodus_data(file_path):
    """
    Reads an Exodus file using meshio. If the main file doesn't exist but split parts do (e.g., filename.e.part1, filename.e.part2, ...),
    concatenates the parts into a temporary file and reads that.
    ...
    if not os.path.exists(file_path):
        ...
        part_pattern = os.path.join(dir_name, f"{base_name}.part*")
        part_files = sorted(glob.glob(part_pattern), key=lambda x: int(os.path.basename(x).split('.part')[-1]))
        
        if not part_files:
            st.error(f"File not found: {file_path}. No split parts found either.")
            return None
        
        # Concatenate parts into a temporary file
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix='.e') as temp_file:
                for part in part_files:
                    with open(part, 'rb') as part_file:
                        temp_file.write(part_file.read())
                temp_path = temp_file.name
        except Exception as e:
            st.error(f"Error concatenating split files: {type(e).__name__}: {e}")
            return None
        
        # Use the temp_path for reading
        read_path = temp_path
    else:
        read_path = file_path
