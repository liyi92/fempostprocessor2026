...
[ICs]
  ...
  [./c]
    ...
    type = FunctionIC
    # function = 'if(x>=0&x<=5,0.2,if(y<30&y>20,0.8,0.2))'
    # function = 'if(x>=0&x<=5,0.2,if(y>40,0.8,if(y>20&y<30,0.8,if(y>0&y<10,0.8,0.2))))'
    # function = 'if(x>=0&x<=8,0.2,if(y>45|y<5,0.8,0.2))'# model 0-c solidmechstructure31 0-c
    # function = 'if(x>=0&x<=8,0.2,if(y<35&y>15,0.8,0.2))' #model 1-A solidmechstructure41
    function = 'if(x>=0&x<=8,0.2,if(y>45,0.8,if(y>10&y<40,0.8,if(y>=0&y<5,0.8,0.2))))'# model 1-B solidmechstructure51
    # function = 'if(x>=0&x<=8,0.2,if(y>35|y<15,0.8,0.2))'# model 1-c solidmechstructure61
  [../]
[]
...
[Materials]
  ...
  [./system_constants]
    ...
    prop_values = '1e-13 5e-13 1e-8 0.25 0.0001 0.1 1e-11 1e-11 1e7 1.19'
  [../]
  [./material_constants]
    type = GenericConstantMaterial
...
[Outputs]
  ...
  file_base = results/solidmechstructure52/test1
[]
